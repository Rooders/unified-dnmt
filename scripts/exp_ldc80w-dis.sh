...
bash $discoures_terms_eval/LTCR/compute_ltcr_awe.sh $discoures_terms_eval/LTCR \
                                       ...
                                       $test_set.src tran $en_stopwords_file $zh_stopwords_file $test_device $python


# Compute APT score
bash $discoures_terms_eval/APT/APT.sh  $sent_model_dir/$best_sent_checkpoint \
                                      $data_dir/$test_set.clean.$src \
                                      $sent_model_dir/$best_sent_checkpoint/$test_set.tran \
                                      $data_dir/$test_set.clean.en0 \
                                      $test_set.$src $test_device $python2 $python

# Compute cohension
$python $discoures_terms_eval/Cohesion/compute_cohesion.py --sent_file $sent_model_dir/$best_sent_checkpoint/$test_set.tran --device $test_device \
                            --doc_file $data_dir/$test_set.document.bpe.$src 2>&1 | tee $sent_model_dir/$best_sent_checkpoint/$test_set.CHS.evalmb

# Compute coherence
$python $discoures_terms_eval/Coherence/compute_coherence.py --sent_file $sent_model_dir/$best_sent_checkpoint/$test_set.tran  \
                            --doc_file $data_dir/$test_set.document.bpe.$src 2>&1 | tee $sent_model_dir/$best_sent_checkpoint/$test_set.CHR.evalmb              
